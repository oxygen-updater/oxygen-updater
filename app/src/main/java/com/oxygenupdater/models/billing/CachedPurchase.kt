 ...
 *
 * - Security: Imagine a situation where four users of Oxygen Updater decide to share one ad-free unlock
 *   purchase amongst themselves. So one user would buy the unlock and then log into the Play Store on each phone
 *   and as soon as they open the Oxygen Updater app on a phone, they would see no ads.
 *   That would be fraudulent, but since this [Entity] is part of the local cache, the [BillingRepository] would notice
 *   there is a purchase in this local cache that the secure server does not recognize as belonging to this user.
 *   The secure server would then conduct further investigations and discover the fraud.
 *
