 ...
 * */
inline fun <LazyState : ScrollableState, LazyStateItem> LazyState.interpolateFirstItemIndex(
...
fun itemVisibilityPercentage(
    ...
    val startOffset = if (itemStartOffset > viewportStartOffset) 0 else abs(
        abs(viewportStartOffset) - abs(itemStartOffset)
    )
    val endOffset = if (itemEnd < viewportEndOffset) 0 else abs(
        abs(itemEnd) - abs(viewportEndOffset)
    )
