...
class SettingsManager(private val context: Context?) {
    ...
    fun <T> getPreference(key: String?, defaultValue: T): T {
        ...
        return when {
            preferences == null -> defaultValue
            preferences.contains(key) -> preferences.all[key] as T
            else -> defaultValue
        }
    }
    ...
    fun savePreference(key: String?, value: Any?) {
        try {
            val editor = sharedPreferencesEditor ?: return
            ...
            editor.apply {
                when (value) {
                    ...
                    is Collection<*> -> {
                        val valuesSet: MutableSet<String> = HashSet()
                        for (item in value) {
                            if (item != null) {
     ...
     */
    fun checkIfSetupScreenIsFilledIn(): Boolean {
     ...
     */
    fun checkIfSetupScreenHasBeenCompleted(): Boolean {
     ...
     */
    fun checkIfOfflineUpdateDataIsAvailable(): Boolean {
        return try {
    ...
    private val sharedPreferences: SharedPreferences?
        get() = if (context == null) null else PreferenceManager.getDefaultSharedPreferences(context)
