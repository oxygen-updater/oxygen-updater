...
object UpdateInstaller {
    ...
    fun installUpdate(
        ...
        addSUCommand(suCommands, ECHO + singleQuoted(recoveryCommands.toString()) + " > " + installScriptPath, false)
    ...
    private fun executeSUCommands(commands: MutableList<String>, context: Context?) {
        ...
        if (commandsOutput.isNullOrEmpty() || commandsOutput[0] != TEXT_SUCCESS) {
            throw UpdateInstallationException(
                if (context != null) context.getString(R.string.install_error_write_script_failed) else "Internal error"
            )
        }
