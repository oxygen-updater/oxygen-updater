...
public class RootInstallLogger extends JobService {
	...
	public boolean onStartJob(JobParameters params) {
		...
		if (deviceId == -1L || updateMethodId == -1L) {
			logError(TAG, new OxygenUpdaterException("Failed to log update installation action: Device and / or update method not selected."));
			return true; // Retrying wont fix this issue. This is a lost cause.
		}
		...
		connector.logRootInstall(installation, result -> {
			if (result == null) {
				logError(TAG, new NetworkException("Failed to log update installation action on server: No response from server"));
				jobFinished(params, true);
			} else if (!result.isSuccess()) {
				logError(TAG, new OxygenUpdaterException("Failed to log update installation action on server: " + result.getErrorMessage()));
				jobFinished(params, true);
			} else if (result.isSuccess()
					&& installation.getInstallationStatus().equals(InstallationStatus.FAILED)
					|| installation.getInstallationStatus().equals(InstallationStatus.FINISHED)) {
