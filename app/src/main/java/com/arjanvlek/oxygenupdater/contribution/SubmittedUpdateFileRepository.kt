 ...
 */
class SubmittedUpdateFileRepository(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun isFileAlreadySubmitted(filename: String?): Boolean {
        return if (filename.isNullOrEmpty()) {
            ...
            DatabaseUtils.queryNumEntries(readableDatabase, TABLE_NAME, "$COLUMN_NAME= ?", arrayOf(filename)) > 0
        }
    }
