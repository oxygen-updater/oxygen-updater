					...
					Consumer<ServerPostResult> callback = new Consumer<ServerPostResult>() {
						...
						public void accept(ServerPostResult serverPostResult) {
							if (serverPostResult == null) {
								...
								if (errorMessage != null && (errorMessage.equals(FILE_ALREADY_IN_DATABASE) || errorMessage
										.equals(FILENAME_INVALID))) {
									...
									FirebaseAnalytics.getInstance(getApplication())
											.logEvent("CONTRIBUTION_NOT_NEEDED", analyticsBundle);
								} else {
									// server error, try again later
									Logger.logError(TAG, new NetworkException("Error submitting update file " + fileName + ": " + serverPostResult
											.getErrorMessage()));
								}
							} else {
								...
								if (fileName != null && fileName.contains(".zip")) {
									...
									settingsManager.savePreference(SettingsManager.PROPERTY_CONTRIBUTION_COUNT, settingsManager
											.getPreference(SettingsManager.PROPERTY_CONTRIBUTION_COUNT, 0) + 1);

									// Log successful contribution
									Bundle analyticsBundle = new Bundle();
									analyticsBundle.putString("CONTRIBUTION_FILENAME", fileName);
									FirebaseAnalytics.getInstance(getApplication())
											.logEvent("CONTRIBUTION_SUCCESSFUL", analyticsBundle);
								}
