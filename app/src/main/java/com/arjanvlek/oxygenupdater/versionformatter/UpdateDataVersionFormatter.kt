...
object UpdateDataVersionFormatter {
    ...
    fun canVersionInfoBeFormatted(versionInfo: FormattableUpdateData?): Boolean {
        ...
        return firstLine.trim { it <= ' ' }.startsWith(OS_VERSION_LINE_HEADING)
    }
    ...
    fun getFormattedVersionNumber(versionInfo: FormattableUpdateData?): String {
        if (!canVersionInfoBeFormatted(versionInfo)) {
            return if (versionInfo?.internalVersionNumber != null
                    && versionInfo.internalVersionNumber != "")
                "V. " + versionInfo.internalVersionNumber
            else
                "Oxygen OS System Update"
        }

        val firstLine = getFirstLine(versionInfo) // Can never be null due to check canVersionInfoBeFormatted
        val firstLineLowerCase = firstLine.toLowerCase()
    ...
    private fun getFirstLine(versionInfo: FormattableUpdateData?): String {
        if (versionInfo?.updateDescription == null || versionInfo.updateDescription == "") {
            return ""
        }

        val reader = BufferedReader(StringReader(versionInfo.updateDescription))
