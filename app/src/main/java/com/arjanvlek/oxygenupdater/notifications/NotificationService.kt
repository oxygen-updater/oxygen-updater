...
class NotificationService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage?) {
        try {
            ...
            val displayDelayInSeconds = Utils.randomBetween(1, settingsManager.getPreference(PROPERTY_NOTIFICATION_DELAY_IN_SECONDS, 1800))
            ...
            val jobId = StreamSupport.stream(AVAILABLE_JOB_IDS)
                    .filter { id ->
                        StreamSupport.stream(scheduler.allPendingJobs)
                                .noneMatch { ji -> ji.id == id }
                    }
                    .findAny()
                    .orElseThrow { RuntimeException("There are too many notifications scheduled. Cannot schedule a new notification!") }

            taskData.putString(DelayedPushNotificationDisplayer.KEY_NOTIFICATION_CONTENTS, ObjectMapper()
