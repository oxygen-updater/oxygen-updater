	...
	public void onMessageReceived(RemoteMessage remoteMessage) {
		try {
			...
			Integer jobId = StreamSupport.stream(AVAILABLE_JOB_IDS)
					.filter(id -> StreamSupport.stream(scheduler.getAllPendingJobs())
							.noneMatch(ji -> ji.getId() == id))
					.findAny()
					.orElseThrow(() -> new RuntimeException("There are too many notifications scheduled. Cannot schedule a new notification!"));

			taskData.putString(DelayedPushNotificationDisplayer.KEY_NOTIFICATION_CONTENTS, new ObjectMapper()
