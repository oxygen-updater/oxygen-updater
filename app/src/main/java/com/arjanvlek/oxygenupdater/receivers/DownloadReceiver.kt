...
class DownloadReceiver(val UIDownloadListener: UpdateDownloadListener) : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.getStringExtra(PARAM_TYPE)) {
            ...
            TYPE_PAUSED -> UIDownloadListener.onDownloadPaused(
                false, intent.getSerializableExtra(PARAM_PROGRESS) as DownloadProgressData
            )
            TYPE_CANCELLED -> UIDownloadListener.onDownloadCancelled()
            TYPE_PROGRESS_UPDATE -> UIDownloadListener.onDownloadProgressUpdate(
                intent.getSerializableExtra(PARAM_PROGRESS) as DownloadProgressData
            )
            TYPE_DOWNLOAD_ERROR -> intent.let {
                val internalError = it.getBooleanExtra(PARAM_ERROR_IS_INTERNAL_ERROR, false)
                val storageError = it.getBooleanExtra(PARAM_ERROR_IS_STORAGE_SPACE_ERROR, false)
                val serverError = it.getBooleanExtra(PARAM_ERROR_IS_SERVER_ERROR, false)
            ...
            TYPE_STATUS_REQUEST -> intent.let {
                ...
                val progress = it.getSerializableExtra(PARAM_PROGRESS) as DownloadProgressData

                when (it.getSerializableExtra(PARAM_STATUS) as DownloadStatus) {
