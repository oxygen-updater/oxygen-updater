...
import com.arjanvlek.oxygenupdater.receivers.DownloadReceiver.Companion.TYPE_VERIFY_COMPLETE
import com.arjanvlek.oxygenupdater.receivers.DownloadReceiver.Companion.TYPE_VERIFY_FAILED
import com.arjanvlek.oxygenupdater.receivers.DownloadServiceRestarter
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.HAS_ERROR
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.HAS_NO_ERROR
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.NOT_ONGOING
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.ONGOING
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.hideDownloadingNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.hideVerifyingNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.showDownloadCompleteNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.showDownloadFailedNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.showDownloadPausedNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.showDownloadingNotification
import com.arjanvlek.oxygenupdater.utils.LocalNotifications.showVerifyingNotification
...
import com.arjanvlek.oxygenupdater.utils.MD5.checkMD5
...
class DownloadService : IntentService(TAG) {
	 ...
	 */
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int) = super.onStartCommand(intent, flags, startId).let {
    ...
    private fun downloadUpdate(updateData: UpdateData?) {
        ...
        if (updateData?.downloadUrl == null) {
            ...
            sendBroadcastIntent(DownloadReceiver.TYPE_DOWNLOAD_ERROR) {
                logError(
                    TAG,
                    UpdateDownloadException(withAppendedStateHistory("Update data is null or has no Download URL"))
                )
        ...
        if (updateData.downloadUrl?.contains("http") == false) {
            ...
            sendBroadcastIntent(DownloadReceiver.TYPE_DOWNLOAD_ERROR) {
                logError(
                    TAG,
                    UpdateDownloadException("Update data has invalid Download URL (" + updateData.downloadUrl + ")")
                )
                it.putExtra(DownloadReceiver.PARAM_ERROR_IS_INTERNAL_ERROR, true)
            }
                ...
                override fun onError(error: Error) {
                    ...
                    when {
                        ...
                        error.isConnectionError -> {
                            ...
                            val progress = settingsManager.getPreference(
                            ...
                            sendBroadcastIntent(DownloadReceiver.TYPE_PROGRESS_UPDATE) {
                                it.putExtra(DownloadReceiver.PARAM_PROGRESS, progressData)
                            }
                        }
                        ...
                        error.isServerError -> {
                            ...
                            sendBroadcastIntent(DownloadReceiver.TYPE_DOWNLOAD_ERROR) {
                                it.putExtra(
                                    DownloadReceiver.PARAM_ERROR_IS_SERVER_ERROR, true
                                )
                            }
    ...
    private fun pauseDownload(downloadId: Int) {
        ...
        if (downloadId != NOT_SET.toInt() && isStateTransitionAllowed(DOWNLOAD_PAUSED)) {
            ...
            logWarning(
                TAG,
                UpdateDownloadException("Not pausing download, invalid download ID provided or not pause-able.")
            )
        }
    }
    ...
    private fun resumeDownload(downloadId: Int, updateData: UpdateData?) {
        ...
        if (!isStateTransitionAllowed(DOWNLOAD_QUEUED)) {
            logWarning(
                TAG,
                UpdateDownloadException("Not resuming download, is a download operation already in progress?")
            )
            return
        }
    ...
    private fun checkDownloadStatus(downloadId: Int, updateData: UpdateData?) {
        logDebug(
            TAG,
            "Checking status for download #$downloadId"
                    + if (updateData != null) " and updateData ${updateData.versionNumber}" else ""
        )
     ...
     */
    private fun isStateTransitionAllowed(newState: DownloadStatus) = when (newState) {
    ...
    private fun deserializeExecutedStateTransitions() = settingsManager.getPreference(
        ...
        if (serializedStateHistory.isEmpty()) {
            ArrayList()
        } else {
    ...
    companion object {
        ...
        private const val NOTIFICATION_CHANNEL_ID = "com.arjanvlek.oxygenupdater.services.DownloadService"
        ...
        fun performOperation(activity: Activity?, action: String?, updateData: UpdateData?) {
            ...
            try {
                ...
                logError(
                    TAG,
                    withAppendedStateHistory("Failed to start DownloadService"), e
                )
                isOperationPending.set(false)
            }
        }

        fun isRunning() = state === DOWNLOADING
                || state === DOWNLOAD_QUEUED
                || state === VERIFYING
                || state === DOWNLOAD_PAUSED_WAITING_FOR_CONNECTION

        private fun withAppendedStateHistory(text: String) =
            "$text\n\nHistory of actions performed by the downloader:\n" + executedStateTransitions.filter {
